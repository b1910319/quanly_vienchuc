...
<div class="row">
  <div class="card-box">
    ...
    <form action="{{ URL::to('/quanlychucvu_xuatfile') }}" method="POST" autocomplete="off" enctype="multipart/form-data">
      {{ csrf_field() }}
        ...
        <tbody  >
          @foreach ($list_vienchuc as $key => $vienchuc)
            <tr >
              ...
              <td style="width: 21%">
                <?php
                  foreach ($count_quatrinhchucvu_vienchuc as $key => $count) {
                    if($count->ma_vc == $vienchuc->ma_vc && $count->sum > 0){
                      ?>
                        <a href="{{ URL::to('/quatrinhchucvu_add/'.$vienchuc->ma_vc) }}">
