...
<div class="row">
  <div class="card-box">
        ...
        <div class="panel panel-default">
          ...
          <div id="collapse1a" class="panel-collapse collapse" role="tabpanel">
            <div class="panel-body mt-3">
              <form action="{{ URL::to('/add_danhmucnghi') }}" method="POST"
                ...
                <div class="row">
                  <div class="col-6">
                    <table class="table">
                      <tbody>
                        <tr>
                          <th scope="row">Tên danh mục nghĩ: </th>
                          <td class="was-validated">
                            <input type='text' id="ten_dmn" class='form-control input_table' autofocus required name="ten_dmn">
                    ...
                    <table class="table">
                      <tbody>
                        <tr>
                          ...
                          <td class="was-validated">
                            <select class="custom-select input_table" id="gender2" name="status_dmn">
    ...
    <div class="mt-3"></div>
    <form action="{{ URL::to('/delete_danhmucnghi_check') }}" method="post" enctype="multipart/form-data">
      {{ csrf_field() }}
            ...
            <tr >
              <td style="width: 5%">
                <div class="form-check">
                  <input class="form-check-input" type="checkbox"  name="ma_dmn[{{ $danhmucnghi->ma_dmn }}]" value="{{ $danhmucnghi->ma_dmn }}">
                </div>
              </td>
              <th scope="row">{{ $key+1 }}</th>
              <td>
                {{ $danhmucnghi->ten_dmn }} ({{ $danhmucnghi->ma_dmn }})
              </td>
              <td>
                <?php
                  if($danhmucnghi->status_dmn == 0){
                    ?>
                      <span class="badge badge-light-success">
                        <i class="fas fa-solid fa-eye"></i>&ensp;  Hiển thị
                      </span>
                    <?php
                  }else if($danhmucnghi->status_dmn == 1) {
                    ?>
              ...
              <td style="width: 27%;">
                <a href="{{ URL::to('/edit_danhmucnghi/'.$danhmucnghi->ma_dmn)}}">
                  <button type="button" class=" btn btn-warning button_cam">
                    <i class="fa-solid fa-pen-to-square text-light"></i>
                    &ensp; Cập nhật
                  </button>
                </a>
                <input class="ma_dmn{{ $danhmucnghi->ma_dmn }}" type="hidden" value="{{ $danhmucnghi->ma_dmn }}">
                <button type="button" class=" xoa{{ $danhmucnghi->ma_dmn }} btn btn-danger button_do"><i class="fa-solid fa-trash text-light"></i> &ensp;Xoá</button>
                <?php
                  if($danhmucnghi->status_dmn == 0){
                    ?>
                      <a href="{{ URL::to('/select_danhmucnghi/'.$danhmucnghi->ma_dmn) }}">
    ...
    document.querySelector('.xoa{{ $danhmucnghi->ma_dmn }}').addEventListener('click', (event)=>{
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
          var id= $('.ma_dmn{{ $danhmucnghi->ma_dmn }}').val();
          $.ajax({
            url:"{{ url("/delete_danhmucnghi") }}", 
            type: "GET", 
            data: {id:id},
          });
          swalWithBootstrapButtons.fire(
...
<script>
  $(document).ready(function(){
    $('#ten_dmn').mouseout(function(){
      var ten_dmn = $(this).val();
      var ten = '';
      // alert(ten_dmn);
      $.ajax({
        url:"{{ url("/check_ten_dmn") }}",
        type:"GET",
        data:{ten_dmn:ten_dmn},
        success:function(data){
          if(data == 1){  
            $('#baoloi').html('Danh mục nghĩ đã tồn tại');
            $('#ten_dmn').val('');
          }else{
            $('#baoloi').html(''); 
          }
