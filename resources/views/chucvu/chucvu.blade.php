...
<div class="row">
  <div class="card-box">
    <div class="alert alert-light color_alert" role="alert">
      ________THÔNG TIN CHỨC VỤ________
    </div>
    <?php 
      $mess = session()->get('message_add_chucvu');
      if ($mess != null) {
        ?>
        ...
        <?php
        $mess = session()->put('message_add_chucvu', null);
      }
    ?>
    <?php 
      $mess = session()->get('message_update_chucvu');
      if ($mess != null) {
        ?>
        ...
        <?php
        $mess = session()->put('message_update_chucvu', null);
      }
    ?>
    <div class="faqs-page block ">
      <div class="panel-group" id="accordion2" role="tablist" aria-multiselectable="true">
        <div class="panel panel-default">
            ...
            <div class="modal-dialog modal-lg">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form class="form-container" action="{{ URL::to('add_chucvu_excel') }}" method="POST" enctype='multipart/form-data'>
                    @csrf
                    <div class="row">
                      <div class="col-9">
                        <div class="mb-3">
          ...
          <a onclick="return confirm('Bạn có muốn xóa tất cả danh mục không?')" href="{{ URL::to('/delete_all_chucvu') }}">
          ...
          <div id="collapse1a" class="panel-collapse collapse" role="tabpanel">
            <div class="panel-body mt-3">
              <form action="{{ URL::to('/add_chucvu') }}" method="POST"
          ...
          @foreach ($list as $key => $chucvu)
            <tr >
              ...
              <td style="width: 27%;">
                ...
                <input class="ma_cv{{ $chucvu->ma_cv }}" type="hidden" value="{{ $chucvu->ma_cv }}">
                <button type="button" class=" xoa{{ $chucvu->ma_cv }} btn btn-danger button_do"><i class="fa-solid fa-trash text-light"></i> &ensp;Xoá</button>
                <?php
                  if($chucvu->status_cv == 0){
                    ?>
...
<script>
  @foreach ($list as $chucvu )
    document.querySelector('.xoa{{ $chucvu->ma_cv }}').addEventListener('click', (event)=>{
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
          var id= $('.ma_cv{{ $chucvu->ma_cv }}').val();
          $.ajax({
            url:"{{ url("/delete_chucvu") }}", 
            type: "GET", 
            data: {id:id},
          });
          swalWithBootstrapButtons.fire(
