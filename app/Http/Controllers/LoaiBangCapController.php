...
use App\Imports\LoaiBangCapImport;
...
class LoaiBangCapController extends Controller
{
  ...
  public function check_ten_lbc(Request $request){
    $this->check_login();
    if($request->ajax()){
  ...
  public function add_loaibangcap_excel(Request $request){
    $this->check_login();
    $ma_vc = session()->get('ma_vc');
    $phanquyen_admin = PhanQuyen::where('ma_vc', $ma_vc)
      ->where('ma_q', '=', '5')
    ...
    $phanquyen_qltt = PhanQuyen::where('ma_vc', $ma_vc)
      ...
      ->first();
    if($phanquyen_admin || $phanquyen_qltt){
      Excel::import(new LoaiBangCapImport, $request->file('import_excel'));
